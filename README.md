...
- **文件与组织**：当前文件夹即轻量工作区；左栏以“文件 / 资料 / 结构”组织文件树、独立资料目录、递归全文搜索和多标签写作；顶部搜索可独立检索当前资料区的路径、可解析正文和网页标题、网址。
- **写作编辑**：支持四级标题、目录、列表、引文、分页符、Emoji、段落书签、图片预览与原图复制、表格、代码块、TeX 公式、公式引用、Mermaid 图，以及连续、单页、双页视图和每封信笺独立的显示大小。
- **研究写作**：资料区按“资料区位置 / 资料 / 网页”管理独立目录中的文件和网页来源；PDF 当前页和常见本地文档预览可临时翻译为简体中文；结构页集中处理大纲、脚注、关联笺记、书签和私域 / 公域文献库。
- **AI 功能**：支持 AI 优化、AI 协作和 AI 起稿；完整信笺既可普通问答，也可生成逐项审阅的标题、正文、表格、Mermaid 与拆分/合并方案，所有修改确认后才提交；“帮助 → AI精灵”则以版本化内置知识回答软件使用问题。
- **双编辑组**：左组专注信笺写作，右组可混排多个信笺和资料标签；两组标签会按空间压缩并支持横向滚动，同时支持排序与跨组移动。
- **本地优先与同步安全**：提供 30 秒恢复缓存、失焦/空闲写回、外部版本检测、冲突副本、本地写作检查、信笺版本历史和 OneDrive 等同步文件夹兼容。
- **沉浸写作**：F11 进入真正全屏，隐藏非正文界面，并通过顶部热区临时唤出完整导航栏。
- **导入、导出与迁移**：顶部“出入”入口提供 Markdown、HTML、TXT、DOCX 导入，PDF、分页 PNG、DOCX、Markdown、HTML、TXT 导出，以及 `.jianprofile` 软件配置备份与安全迁移。
