...
export const RELEASE_NOTES = [
  {
    ...
    scale: "major",
    date: "2026-07-26",
    title: "资料检索、轻量 AI 与更新体验完善",
    changes: [
      "顶部搜索新增“资料搜索”，可检索当前资料区的文件名、相对路径、可解析正文，以及公区和当前工作区私区网页的标题与网址；大型资料库会显示可取消的索引进度。",
      "资料正文解析迁移到独立 worker，并加入版本化派生缓存、增量失效、资源上限和损坏文件隔离，避免大型 PDF、DOCX 阻塞写作界面。",
      "更新下载状态改用可访问的进度环展示准备、下载百分比、已下载量和速度，下载过程不再反复占用通用消息提示。",
      "正文选区工具栏新增信笺级多会话 AI 问答；窗口支持拖动、会话切换和最小化为右下角 AI 小精灵继续后台生成，会话仅在本次运行中保留。",
      "AI 任务模型改为默认折叠的独立卡片，可同时展开多项并保留参数草稿；“选区问答”仍可跟随默认模型或精确指定已测试模型。",
      "完成前端、Electron 与平台桥的渐进式模块化拆分，在保持现有界面、快捷键、文件格式和 IPC 契约兼容的前提下收紧领域边界。",
    ],
  },
  {
    version: "0.9.11",
    ...
    title: "资料格式扩展与稳定性修复",
    changes: [
      "资料区新增 DOCX 文件直接打开与阅读，支持标题、段落、列表、表格、图片、页内搜索和缩放，并沿用安全的文档转换与内容清洗。",
      "修复停止 AI 生成后仍可能收到迟到内容的问题，并加强请求超时、取消、重定向和异常响应大小限制。",
      "修复双编辑组下导出目标可能选错的问题；来自未来格式的只读信笺现在会在编辑、模板、引用和保存链路中保持只读。",
      "修复快速切换文件夹或资料库时旧结果覆盖新结果，以及文件监听延迟启动后接管新目录的问题。",
      "保存、恢复缓存和外部修改状态改为按标签独立管理，避免状态串页，保存期间继续编辑也不会被误判为已保存。",
      "浏览器存储不可用时不再伪报文档写入成功，并新增可恢复的界面错误页与安全偏好重置。",
      "统一弹窗焦点循环、关闭后的焦点归还和后台快捷键隔离，并加入真实浏览器交互回归测试。",
    ],
  },
  {
    version: "0.9.10",
