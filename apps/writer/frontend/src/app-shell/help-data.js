...
export const HELP_TOPICS = [
  {
    ...
    steps: ["在__文件__中选择写作文件夹，展开目录并打开 `.letterpaper` 信笺。", "在__资料__中选择独立资料文件夹，管理资料文件和网页来源。", "在__结构__中查看当前信笺的大纲、脚注、关联笺记、写作检查、文献和书签。"],
  ...
  },
  {
    ...
    illustrations: [
      { type: "page-view", alt: "信笺右键菜单展开页面视图二级菜单，完整显示连续、单页和双页三个选项。", caption: "页面视图只改变当前信笺的阅读方式，不改写正文或导出结果。" },
    ],
    steps: ["点击标签切换内容；拖动标签可在组内排序，也可移到另一组。", "通过标签右键菜单把信笺移到左组或右组；资料只在右组打开。", "在信笺页面中右键，展开__页面视图__并选择连续、单页或双页；分页视图可按页跳转、前后翻页和缩放。", "拖动中间分隔线调整宽度；关闭右组最后一个标签后恢复单栏。"],
    tips: ["左组至少保留一个信笺，同一信笺不会同时出现在两个组中。", "双页视图会使用完整写作区域并暂时收起右组；切回连续或单页后可恢复双编辑组。", "标签顺序、活动项、分栏比例、页面视图和资料阅读位置都会在本次会话中分别保留。"],
  },
  {
    id: "save-recovery",
    categoryId: "files",
  ...
  },
  {
    ...
    steps: ["先保存信笺，再在信笺页面中右键并选择__版本历史__。", "从左侧选择版本，查看标题、作者、排版、引用、媒体和正文差异；需要时可修改版本名称。", "确认后恢复所选版本；恢复完成后，信笺会重新载入并保留恢复前安全版本。"],
    tips: ["连续保存相同内容不会重复制造无变化版本；AI 直接应用前也会创建本地安全版本。", "删除单个版本或全部清除均不可撤销；历史只保存在本机，不随 `.letterpaper` 或配置迁移包移动。"],
  },
  {
    id: "interchange",
    categoryId: "files",
    ...
    steps: ["从__出入 → 导入文档__打开 Markdown、HTML、TXT 或 DOCX；导入结果始终成为未保存的新信笺。", "选择__导出信笺__，可输出 PDF、分页 PNG、DOCX、Markdown、HTML 或 TXT。", "需要移动软件设置时，选择__备份与迁移__创建或导入 `.jianprofile` 配置包。"],
    tips: ["通用导出不包含评注和 AI 记录；脚注、引用、公式、代码和图表会按目标格式输出或安全降级，参考文献由顶部__参考__开关决定。", "Markdown 图片写入同名 `.assets` 目录；需要最高视觉保真时请使用 PDF。"],
  },
  {
    ...
    steps: ["从__出入 → 备份与迁移__选择导出，生成带清单与完整性校验的 `.jianprofile` 配置包。", "如确需携带 HTTP 供应商 API Key，可明确开启并设置至少 12 位口令；密钥使用 scrypt 与 AES-256-GCM 加密。", "导入时先验证包、按类别预览新增、变化和冲突，再只勾选需要合并的配置。"],
    tips: ["默认不导出绝对路径、最近文件、恢复文件、历史版本、日志、缓存、Emoji 最近记录、OAuth 或 Codex 登录状态。", "术语冲突会保留本机规则；HTTP 模型导入后会重置为未测试，需要重新连接测试。"],
  },
  {
    id: "search",
    categoryId: "files",
    ...
    steps: ["按 `Ctrl+F` 查找当前信笺，使用上下按钮或 Enter 在匹配项间移动。", "展开替换或按 `Ctrl+H`，可替换当前匹配或一次替换全部。", "按 `Ctrl+P` 搜索当前文件夹及全部子文件夹，选择结果即可打开并定位。", "从顶部搜索菜单选择__资料搜索__，检索当前资料目录、公区网页和当前工作区私区网页。", "等待索引完成后选择结果：信笺、PDF 和可解析文本会尽量定位首个命中位置，网页来源则在现有资料网页视图中打开。"],
    tips: ["资料索引超过约 300ms 才显示进度；可随时取消，大文件超限时仍保留名称和路径搜索。", "可解析正文包括信笺、PDF、DOCX、Markdown、纯文本和表格；图片及不支持格式只搜索名称和路径。", "资料网页只搜索标题和网址，不抓取网页正文、备注或摘要；只读信笺可以查找，但不能执行替换。"],
  },
  {
    id: "editor-outline",
    categoryId: "writing",
    ...
    steps: ["在页面中直接编辑标题、署名、日期和正文。", "使用顶部__标题__菜单设置一至四级标题，并按需覆盖当前标题编号。", "打开__结构 → 大纲__跳转章节，或点击顶部__目录__生成和关闭正文目录。"],
  ...
  },
  {
    ...
    illustrations: [
      { type: "selection-ai-assistant", alt: "现版本选区问答窗口显示会话标签、单行选区快照、移动与最小化操作，以及问题输入区。", caption: "每次“问 AI”都会建立独立会话；窗口可移动、最小化，并在同一信笺中切换多个会话。" },
    ],
    steps: ["框选文字，使用悬浮条设置**加粗**、斜体、__下划线__、颜色和水彩标注。", "点击悬浮条中的__问 AI__，为冻结的选区创建独立临时会话；再次选择文字可继续新建会话。", "拖动标题栏调整窗口位置，或将窗口最小化为信笺右下角的 AI 小精灵；后台生成完成后可从小精灵恢复。", "在媒体菜单中插入链接；已有链接可单击编辑，按 Ctrl/Command 单击打开。", "完整 AI 问答模式下仍可把选区加入“已标记文字”。"],
    tips: ["轻量选区问答只发送选中文字、当前问题和当前会话历史，不会读取其他正文或资料；多个会话只在本次运行中按信笺保留。", "Esc 会先关闭完整快照浮层，否则最小化问答窗口，不会删除会话。", "只读信笺仍可使用安全的选区问答入口，但不会显示或执行任何格式修改操作。"],
  },
  {
    id: "comments",
    categoryId: "writing",
  ...
  },
  {
    ...
    steps: ["从__元素__选择表情，按中英文搜索或分类浏览后插入标准 Unicode Emoji。", "选择__书签__为当前段落添加定位点，再到__结构 → 书签__跳转、改名或删除。", "选择__代码块__并设置语言与是否换行；选择__公式__输入 TeX，可插入行内公式或带编号、标签的块公式。", "块公式可复制同一信笺内有效的公式引用；Mermaid 图支持源码实时预览、图注与小、中、大、满四档宽度。"],
    tips: ["专业内容会随 `.letterpaper` 保留可编辑信息；导出到能力较弱的格式时会采用清晰的静态或文本降级。", "Mermaid 预览失败不会清空源码；超长源码、危险链接与不安全渲染会被拦截。"],
  },
  {
    ...
    steps: ["打开__结构 → 检查__查看当前激活信笺的建议，点击卡片可定位对应文字。", "对单条建议选择忽略一次、替换或全文替换；也可在正文右键把词加入词典。", "点击检查页右上角齿轮，选择中文、英文或中英双语，并维护白名单与“原写法 → 推荐写法”规则。"],
    tips: ["拼写词典、白名单和用词规范均在本机运行；关闭检查不会修改正文，也不会删除已有规则。", "自定义规则可设置启用状态、大小写与完整单词匹配，并可随 `.jianprofile` 迁移。"],
  },
  {
    ...
    steps: ["把光标放在目标位置，从__元素__选择脚注，输入内容后插入。", "选择文献引用，从当前信笺私域或所有信笺共享的公域中搜索，也可新增来源并填写本次引用页码。", "进入__结构 → 文献__，从文献库齿轮管理私域与公域，导入或导出 BibTeX、RIS、CSL-JSON，并设置当前信笺的引用样式与语言。", "点击顶部__参考__开关，在文尾生成或关闭自动参考文献。"],
    tips: ["内置 GB/T 7714 数字制与作者-年份制、APA 7、MLA 9 和 Chicago Author-Date；也可导入 CSL 样式。", "DOI / ISBN 补全只向书目服务发送所填标识符，不会发送正文、笔记或文献库；脚注和引用编号会随正文顺序动态更新。"],
  },
  {
    id: "related-notes",
    categoryId: "writing",
  ...
  },
  {
    ...
    steps: ["在资料树中打开文件或网页，内容会进入右编辑组的新标签。", "PDF 支持页码跳转、方向键和 PageUp/PageDown 翻页、文字搜索、缩放与系统打开。", "搜索 PDF 后，工具栏显示“当前项/总数”；黄色标出本页全部命中，陶橙色标出当前命中，使用上下按钮、Enter 或 Shift+Enter 循环跳转。", "网页可后退、前进、刷新或在系统浏览器打开；其他支持格式提供搜索、缩放或表格滚动。"],
    tips: ["PDF 当前页可作为新建文献引用的默认页码。", "没有文本层的扫描版 PDF 会明确提示先进行 OCR，本版不会对图片内容自动识字。", "不支持内嵌预览的文件会显示安全信息页，可选择用系统应用打开。"],
  },
  {
    id: "ai-modes",
    categoryId: "ai",
    title: "AI 模式选择与模型切换",
    summary: "点击顶部__AI模式__，在[[AI 优化、AI 问答和 AI 起稿]]之间选择当前任务。",
    illustration: "ai-modes",
    illustrationAlt: "AI 模式选择窗口显示 AI 优化、AI 问答和 AI 起稿三张功能卡片。",
    illustrationCaption: "先选择任务，再在模式内切换已测试可用的模型。",
    illustrations: [
      { type: "ai-model-chooser", alt: "AI 问答的模型选择器完整列出可用供应商和模型。", caption: "进入任务后仍可切换已测试成功的模型。" },
    ],
    steps: ["先在设置中配置并测试至少一个可用模型。", "点击__AI模式__，选择 AI 优化、AI 问答或 AI 起稿。", "进入优化或问答后可切换模型；AI 起稿使用任务模型并在独立工作流中生成新信笺。", "退出 AI 模式会返回普通双编辑组布局。"],
    tips: ["模型不可用时会引导打开 AI 配置，不会静默发送请求。", "优化结果、问答记录和输入草稿都按信笺保存；AI 起稿任务会在本机保留必要的恢复状态。"],
  },
  {
    id: "ai-compose",
    categoryId: "ai",
    ...
    steps: ["选择__AI模式 → AI 起稿__，填写主题、读者、目标字数、文章类型、表达风格和补充要求。", "按需展开参考资料，明确勾选当前信笺、资料区内容或文献条目，并核对预计上下文用量。", "点击__生成全稿__，等待结构设计、分节撰写、全文检查和生成信笺依次完成。"],
    tips: ["生成结果会作为新的派生信笺打开，当前信笺不会被覆盖。", "任务失败或停止后会保留已经完成的后台阶段；可返回修改简报，或从中断处继续生成。", "从结构到检查统一使用“AI 起稿”任务模型；未单独指定时跟随默认模型。"],
  },
  {
    id: "ai-providers",
    categoryId: "ai",
    ...
    illustrationAlt: "AI 配置页面显示供应商模型、请求参数、选区问答、直接应用定位和 AI 起稿任务模型。",
    illustrationCaption: "只有测试成功的模型可用于写作任务或设为任务专用模型。",
    steps: ["从设置进入__AI 配置__，选择 Gemini、DeepSeek、Codex CLI 或自定义供应商。", "为 HTTP 模型填写模型标识和请求参数，测试成功后设为默认。", "打开__任务模型__，按需为选区问答、直接应用定位或 AI 起稿指定专用模型；未指定时跟随默认模型。"],
    tips: ["任务卡重新打开设置时默认折叠，可同时展开多项；折叠不会丢失本次设置会话中的参数草稿。", "AI 起稿从结构设计到正文生成、全文检查统一使用同一个任务模型。", "选区问答只使用冻结的选中文字和当前会话历史；直接应用定位模型只判断替换或插入位置。", "API Key 仅保存在本机；切换任务供应商时会清空不兼容的任务请求参数。"],
  },
  {
    id: "codex-cli",
    categoryId: "ai",
  ...
  },
  {
    ...
    tips: ["研究资料即使在右侧打开，也不会自动进入 AI 请求；只有 AI 起稿中明确勾选的资料才会随该任务发送。", "轻量选区问答只携带冻结选区和当前会话历史。", "原图会增加 token；图片失效时会明确报告图号，不会静默跳过。"],
  },
  {
    id: "focus-mode",
    categoryId: "view",
  ...
  },
  {
    ...
    illustrations: [
      { type: "release-notes", alt: "点击版本号后打开完整更新历史，左侧为阶段列表，右侧为版本详情。", caption: "应用更新历史内置在笺间中，无需联网也可查看。" },
    ],
    steps: ["左侧查看字数、段落、页数和图片统计。", "中间确认恢复缓存写入状态，并查看或清理信笺切换缓存。", "右侧检查更新；点击版本号可离线查看完整应用更新历史。"],
    tips: ["恢复缓存用于意外恢复，编辑器结构缓存只用于加速标签切换。", "浏览器预览不支持真实更新；桌面版下载更新时会在状态栏用进度环显示准备状态、百分比、已下载量和速度。"],
  },
];
