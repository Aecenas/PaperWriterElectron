...
export const HELP_TOPICS = [
  ...
  },
  {
    ...
    illustrationAlt: "AI 配置页面显示供应商模型、请求参数、选区问答和直接应用定位任务模型。",
    illustrationCaption: "只有测试成功的模型可用于写作任务或设为任务专用模型。",
    steps: ["从设置进入__AI 配置__，选择 Gemini、DeepSeek、Codex CLI 或自定义供应商。", "为 HTTP 模型填写模型标识和请求参数，测试成功后设为默认。", "打开__任务模型__，按需展开选区问答或直接应用定位卡片并指定专用模型；未指定时跟随默认模型。"],
    tips: ["任务卡重新打开设置时默认折叠，可同时展开多项；折叠不会丢失本次设置会话中的参数草稿。", "选区问答只使用冻结的选中文字和当前会话历史；直接应用定位模型只判断替换或插入位置。", "API Key 仅保存在本机；切换任务供应商时会清空不兼容的任务请求参数。"],
  },
  {
    id: "codex-cli",
    categoryId: "ai",
  ...
  },
  {
    ...
    tips: ["研究资料即使在右侧打开，也不会进入 AI 请求；轻量选区问答只携带冻结选区和当前会话历史。", "原图会增加 token；图片失效时会明确报告图号，不会静默跳过。"],
  },
  {
    id: "focus-mode",
    categoryId: "view",
