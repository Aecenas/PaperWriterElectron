...
test("release phase ranges and major/minor totals are derived from release data", () => {
  ...
  assert.equal(RELEASE_PHASES.reduce((total, phase) => total + phase.minorCount, 0), 17);
  assert.deepEqual(
    RELEASE_PHASES.map((phase) => [phase.id, phase.majorCount, phase.minorCount, phase.versionRange]),
    [
      ["professional-creation", 2, 6, "V1.1.0 — V1.2.0"],
...
test("the 0.9.8 consolidation remains intact after the 1.2.0 release", () => {
  const versions = RELEASE_NOTES.map((release) => release.version);
  assert.equal(versions[0], "1.2.0");
