...
const editorCommandsSource = await readFile(new URL("./editor/commands.js", import.meta.url), "utf8");
const aiToolbarsSource = await readFile(new URL("./ai/Toolbars.jsx", import.meta.url), "utf8");
...
test("clearing the finalized break removes only that node in one transaction", () => {
  const removeFinalized = sourceBetween("function removeFinalizedBreak", "function insertTableOfContents", editorCommandsSource);
...
test("the finalized-line toolbar action toggles between insert and clear", () => {
  const toolbar = sourceBetween("function AiOptimizeToolbar", "function AiChatToolbar", aiToolbarsSource);
